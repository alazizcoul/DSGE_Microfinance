...
exp(R_b) = exp(r_ib) - kappa_km * ( exp(K_m) / exp(B) - exp(vi) ) * (exp(K_m)/exp(B))^2   ; // (26) FOC wholesale branch, where r_ib is policy rate

exp(K_m) * exp(pie) = (1-deltakm) * exp(K_m(-1)) / exp(eps_K_m) + omega_m* exp(j_m(-1)) ; // (27) accumulation of bank capital

gamma_b * exp(d_b)  = gamma_p * exp(d_p) ;                                      
gamma_b * exp(b_h)  = gamma_i* exp(b_i) ; 
gamma_b * exp(b_e)  = gamma_e * exp(b_ee); 

exp(b_h) + exp(b_e)  =  exp(d_b) + exp(K_m) ;
